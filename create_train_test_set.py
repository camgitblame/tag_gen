...
def format_for_gpt2(df):
    ...
    # Optional: filter overly long examples
    return df[df["formatted"].str.len() < 1000]
...
def create_balanced_train_test(input_path, train_out, test_out, test_frac=0.2, random_seed=42):
    ...
    df["genre"] = df["parsed_genres"].apply(lambda g: ", ".join(g))
    ...
    formatted_test = test_df[test_df["overview"].notna()
                             & test_df["tagline"].notna()]

    formatted_train["formatted"].to_csv("train.txt", index=False, header=False)
    formatted_test[["title", "overview", "tagline"]].to_csv(
        "eval.csv", index=False)
    formatted_test[["title", "overview", "tagline", "genre"]].to_csv(
        "eval_with_genre.csv", index=False)
